(() => {
  ...
  const steps=[
    ...
    {id:"favoriteMeal",type:"choice",title:"Qual refeição do dia você mais gosta?",options:[c("Café da manhã"),c("Almoço"),c("Jantar"),c("Lanches"),c("Não tenho uma refeição favorita")]},
    ...
    {id:"allergies",type:"multi",title:"Você possui alguma alergia ou intolerância alimentar?",options:[c("Nenhuma"),c("Lactose"),c("Oleaginosas"),c("Frutos do mar"),c("Proteína do leite"),c("Proteína do ovo"),c("Mel"),c("Cítricos"),c("Outra")]},
  ...
  const option=(x,i,selected,image,icon)=>`<button class="answer ${selected?"selected":""} ${image?"image-answer":""} ${icon&&x.image?"icon-answer":""}" data-option="${i}" aria-pressed="${selected}">${image?`<div class="answer-image">${asset(x.image,"")}</div>`:icon&&x.image?`<span class="mini-picture">${asset(x.image,"")}</span>`:""}<span class="answer-label">${esc(x.label)}</span><span class="answer-check">✓</span></button>`;
  const testimonials=(amount=2,start=1)=>`<section class="testimonial-area"><h2>O que as pessoas dizem:</h2><div class="testimonial-grid">${Array.from({length:amount},(_,i)=>{const number=start+i,path=ASSETS.comments[number-1];return `<article class="testimonial-slot"><img src="${esc(path)}" alt="Comentário ${number} sobre a experiência com o plano Carnívora 40+" width="720" height="480" loading="lazy" decoding="async"></article>`}).join("")}</div></section>`;
  ...
  function renderOffer(){
    ...
    app.innerHTML=`<main class="offer-page">
      <section class="offer-hero"><div class="offer-container offer-narrow">${logo()}${assetPath("offerEvolution")?`<div class="offer-progress-image">${asset("offerEvolution","Comparação visual entre a condição atual e a meta")}</div>`:""}<div class="condition"><div><span>Sua condição atual</span><strong>25%</strong></div><i>→</i><div><span>Seu potencial de consistência</span><strong>90%</strong></div></div>${timer()}<h1>${name?`${name}, seu`:"Seu"} plano alimentar personalizado está pronto!</h1><p class="hero-copy">Organizamos suas respostas em uma experiência prática para acompanhar pelo celular.</p>${buyBox()}</div></section>
      ...
      <section class="visual-section"><div class="offer-container offer-narrow"><p class="eyebrow">UMA EXPERIÊNCIA VISUAL E SIMPLES</p><h2>Veja as mudanças que uma rotina consistente pode apoiar</h2><div class="wide-image">${asset("visibleChange","Imagem de antes e depois")}</div></div></section>
      <section><div class="offer-container offer-narrow"><p class="eyebrow">HISTÓRIAS REAIS</p><h2>Veja a experiência de quem já começou</h2>${testimonials(4,9)}${timer()}${buyBox()}</div></section>
      <section class="faq-section"><div class="offer-container offer-narrow"><h2>Ficou com alguma dúvida?</h2><p class="centered">Veja as perguntas mais frequentes</p><div class="faq"><details open><summary>O que encontrarei no plano?</summary><p>Refeições, receitas, organização de compras, metas e conteúdos complementares.</p></details><details><summary>Como minhas respostas são utilizadas?</summary><p>Elas ajudam a organizar a experiência conforme seu objetivo, rotina e preferências registradas.</p></details><details><summary>Quando recebo o acesso?</summary><p>O acesso é liberado conforme as informações exibidas no checkout após a confirmação do pagamento.</p></details><details><summary>Consigo acompanhar pelo celular?</summary><p>Sim. O conteúdo foi estruturado para consulta em telas de celular.</p></details><details><summary>Preciso fazer exercícios?</summary><p>A atividade física é complementar e deve respeitar seus limites e a orientação de um profissional.</p></details><details><summary>Os resultados são iguais para todo mundo?</summary><p>Não. Resultados variam conforme rotina, características individuais e constância.</p></details></div></div></section>
      <section><div class="offer-container offer-narrow guarantee"><div class="guarantee-image">${asset("guarantee","Imagem autorizada da garantia")}</div><div><p class="eyebrow">COMPRA PROTEGIDA</p><h2>Garantia de reembolso</h2><p>Consulte no checkout o prazo e todas as condições aplicáveis à garantia.</p></div></div></section>
      <section class="last-call"><div class="offer-container offer-narrow"><p class="eyebrow">SEU PLANO JÁ ESTÁ PRONTO</p><h2>Comece quando estiver preparado</h2><p class="centered">Revise os detalhes e conclua o acesso pelo ambiente seguro de pagamento.</p>${timer()}${buyBox()}</div></section>
