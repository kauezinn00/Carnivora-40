(() => {
  ...
  const socialProofItems=()=>ASSETS.socialProofSlides?.[state.answers.gender]||[assetPath("socialProof")].filter(Boolean);
  const socialProofCarousel=()=>{const items=socialProofItems();if(!items.length)return "";return `<div class="social-proof-carousel" data-social-proof-carousel><div class="social-proof-track" data-social-proof-track>${items.map((path,i)=>`<article class="social-proof-slide" aria-label="Prova social ${i+1} de ${items.length}"><img src="${esc(path)}" alt="Prova social ${i+1} com relato de transformação" loading="lazy" decoding="async"></article>`).join("")}</div>${items.length>1?`<div class="social-proof-dots">${items.map((_,i)=>`<button class="social-proof-dot ${i===0?"is-active":""}" type="button" data-social-proof-dot="${i}" aria-label="Mostrar prova social ${i+1}"></button>`).join("")}</div>`:""}</div>`};
  ...
  let offerTimerId;
  let socialProofAutoplayIds=[];
  function startSocialProofCarousels(){
    socialProofAutoplayIds.forEach(id=>clearInterval(id));
    socialProofAutoplayIds=[];
    document.querySelectorAll('[data-social-proof-carousel]').forEach(carousel=>{
      const track=carousel.querySelector('[data-social-proof-track]');
      const slides=[...carousel.querySelectorAll('.social-proof-slide')];
      const dots=[...carousel.querySelectorAll('[data-social-proof-dot]')];
      if(!track||slides.length<=1)return;
      let index=0;
      const updateDots=(active)=>dots.forEach((dot,i)=>dot.classList.toggle('is-active',i===active));
      const slideWidth=()=>carousel.clientWidth;
      const goTo=(nextIndex,smooth=true)=>{
        index=(nextIndex+slides.length)%slides.length;
        track.scrollTo({left:slideWidth()*index,behavior:smooth?'smooth':'auto'});
        updateDots(index);
      };
      let scrollTick;
      track.addEventListener('scroll',()=>{
        ...
        scrollTick=setTimeout(()=>{
          index=Math.round(track.scrollLeft/Math.max(1,slideWidth()));
          updateDots(index);
        },60);
      },{passive:true});
      ...
      const start=()=>{stop();autoplayId=setInterval(()=>goTo(index+1),4500);};
      ...
      start();
      socialProofAutoplayIds.push(autoplayId);
    });
  }
  ...
  function renderOffer(){
    ...
    app.innerHTML=`<main class="offer-page">
      ...
      <section class="what-you-get-section"><div class="offer-container offer-narrow"><p class="eyebrow">SEU PLANO COMPLETO</p><h2>Veja tudo que você recebe no Carnívora 40+</h2><p class="section-intro">Tudo organizado para você saber o que fazer sem precisar montar o plano sozinho.</p><ul class="receive-list"><li><span class="receive-check">✓</span><span class="receive-emoji">🥩</span><div><strong>Plano alimentar personalizado</strong><p>Montado com base no seu objetivo e na sua rotina.</p></div></li><li><span class="receive-check">✓</span><span class="receive-emoji">🛒</span><div><strong>Lista prática de compras</strong><p>Saiba exatamente o que comprar sem perder tempo.</p></div></li><li><span class="receive-check">✓</span><span class="receive-emoji">🍳</span><div><strong>Sugestões de refeições simples</strong><p>Ideias fáceis para seguir o plano no dia a dia.</p></div></li><li><span class="receive-check">✓</span><span class="receive-emoji">🔁</span><div><strong>Opções de substituição</strong><p>Troque alimentos sem bagunçar sua estratégia.</p></div></li><li><span class="receive-check">✓</span><span class="receive-emoji">📲</span><div><strong>Acesso imediato no celular</strong><p>Consulte seu plano onde e quando quiser.</p></div></li><li><span class="receive-check">✓</span><span class="receive-emoji">📘</span><div><strong>Passo a passo para começar</strong><p>Tudo organizado para você aplicar sem dúvida.</p></div></li></ul></div></section>
      ...
      <section class="social-proof-section"><div class="offer-container offer-narrow"><p class="eyebrow">EXPERIÊNCIA EM DESTAQUE</p><h2>Veja a experiência de quem já começou</h2>${socialProofCarousel()}<p class="social-proof-note">Resultados podem variar de pessoa para pessoa.</p>${timer()}${buyBox()}</div></section>
    ...
    window.scrollTo({top:0});startOfferTimer();startSocialProofCarousels();
  }
